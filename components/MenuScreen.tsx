...
export const MenuScreen: React.FC<MenuScreenProps> = ({
    ...
    return (
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setTwoPlayerLevel(TWO_PLAYER_LEVELS.find((l) => l.level === parseInt(e.target.value))!)
          }
  ...
  return (
    <div className="w-full max-w-md bg-gray-800/50 backdrop-blur-sm p-8 rounded-xl shadow-2xl border border-cyan-500/20">
      <h1
        className={`text-6xl md:text-7xl font-orbitron font-black text-center mb-8 bg-clip-text text-transparent bg-gradient-to-r ${theme.titleGradient}`}
      >
        tikiTaP0
      </h1>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`p-4 rounded-lg font-bold transition ${mode === GameMode.Offline ? `${theme.accent1Bg} ring-2 ring-white/50` : 'bg-gray-700 hover:bg-gray-600'}`}
            >
              vs CPU
            </button>
            <button
              onClick={() => handleModeSelect(GameMode.AI)}
              className={`p-4 rounded-lg font-bold transition ${mode === GameMode.AI ? `${theme.accent1Bg} ring-2 ring-white/50` : 'bg-gray-700 hover:bg-gray-600'}`}
            >
              vs AI
            </button>
            <button
              onClick={() => handleModeSelect(GameMode.TwoPlayer)}
              className={`p-4 rounded-lg font-bold transition ${mode === GameMode.TwoPlayer ? `${theme.accent1Bg} ring-2 ring-white/50` : 'bg-gray-700 hover:bg-gray-600'}`}
            >
              2 Player
            </button>
            <button
              onClick={() => handleModeSelect(GameMode.Online)}
              className={`p-4 rounded-lg font-bold transition ${mode === GameMode.Online ? `${theme.accent1Bg} ring-2 ring-white/50` : 'bg-gray-700 hover:bg-gray-600'}`}
            >
              Online
            </button>
          </div>
          {mode === GameMode.AI && (
            <p className="text-center text-xs text-gray-400 mt-1">
              (Win to get a Gemini-generated image!)
            </p>
          )}
        </div>
      ...
      <div className="mt-8 grid grid-cols-3 gap-2">
        ...
        <button
          ...
          className="w-full py-3 bg-gray-700 hover:bg-blue-800 rounded-lg text-sm font-orbitron font-bold transition-all transform hover:scale-105"
        >
          LEADERBOARD
        </button>
        <button
