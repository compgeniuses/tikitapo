...
const App: React.FC = () => {
  ...
  const handleCellClick = (row: number, col: number) => {
    ...
    if (
      ...
      (!isMyTurnOnline && !isMyTurnOffline) ||
      winner
    )
      return;
  ...
  const handleResetProgress = () => {
    if (
      window.confirm(
        'Are you sure you want to reset your level progress? This will lock all levels except the first one in each difficulty.'
      )
    ) {
  ...
  const renderContent = () => {
    ...
    switch (gameState) {
      ...
      case GameState.Settings:
        return (
          <SettingsScreen
            ...
            onGoToAvatarCreation={handleGoToAvatarCreation}
          />
        );
      ...
      case GameState.AgeCheck:
        return (
          <AgeCheckScreen
            question={ageCheckAnswer}
            onSuccess={handleAgeCheckSuccess}
            onBack={() => setGameState(GameState.ProfileSelect)}
          />
        );
      ...
      case GameState.Leaderboard:
        return <LeaderboardScreen onBack={() => setGameState(GameState.Menu)} theme={theme} />;
      case GameState.OnlineLobby:
        return (
          <OnlineLobbyScreen
            ...
            lobbies={lobbies}
            myName={settings.playerNames[Player.X]}
            myAvatarUrl={settings.avatarUrl}
          />
        );
              ...
              <GameBoard
                ...
                disabled={
                  (gameMode === GameMode.Online
                    ? currentPlayer !== myPlayerPiece
                    : isComputerTurn) || gameState !== GameState.Playing
                }
                theme={theme}
              />
              {isAiThinkingMove && (
                <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col items-center justify-center rounded-lg z-20">
                  ...
                  <h2 className="text-2xl font-orbitron font-bold text-purple-300 mt-6">
                    {difficulty.toUpperCase()} AI IS THINKING...
                  </h2>
                </div>
              )}
              {gameState === GameState.Paused && gameMode !== GameMode.Online && (
                <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col items-center justify-center rounded-lg z-20">
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-green-500 hover:bg-green-400 rounded-md text-xl font-bold transition-transform transform hover:scale-105"
                  >
                    RESUME
                  </button>
                </div>
  ...
  return (
    <main
      className={`min-h-screen text-white flex items-center justify-center p-4 transition-colors duration-500 ${currentBackground}`}
    >
      {renderContent()}
